...
#ifndef __CAN_NODE_FANS_BUS0_H__
#define __CAN_NODE_FANS_BUS0_H__
#include <stdint.h>
#include <string.h>
#include <ctype.h>
...
{
    uint8_t ctrl_to_fan;
} t_can_node_fans_bus0_input_alive;
...
typedef struct
{
    ...
    t_can_node_fans_bus0_input_alive alive;
} t_can_node_fans_bus0_input;

typedef struct
{
    uint32_t fan_status;
    uint32_t fan_power;
} t_can_node_fans_bus0_output_tx_now;
...
typedef struct
{
    ...
    t_can_node_fans_bus0_output_tx_now tx_now;
} t_can_node_fans_bus0_output;
...
void can_node_fan_power_cb(uint32_t id, uint64_t msg, uint32_t dlc);

void can_node_fan_status_checksum_cb(uint32_t id, uint64_t msg, uint32_t dlc);
void can_node_fan_power_checksum_cb(uint32_t id, uint64_t msg, uint32_t dlc);
